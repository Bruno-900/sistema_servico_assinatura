...
public class AssinaturaDAO {
    ...
    public void cadastrarAssinatura(Assinatura novaAssinatura){
        String sql = "INSERT INTO assinatura (status, data_inicio, data_Fim) VALUES (?,?,?,?)";
    ...
    public void atualizarAssinatura(Assinatura atualizarAssinatura){
        String sql = "UPDATE assinatura SET status = ? data_inicio = ?, data_fim = ? WHERE id_assinatura = ?";
        try (Connection conexao = ConexaoBanco.getConexao();
             PreparedStatement declarando = conexao.prepareStatement(sql)) {
            ...
            declarando.setInt(4, atualizarAssinatura.getIdAssinatura());
